...
Code Organization and Role of Each File

Top-level structure (this module)
...
Uses requests, BeautifulSoup, and urllib3 to find the main “Rivers” PDF link for each year and save it into data/CPCB_Rivers_Main_Data/<year>.pdf.
...
Uses camelot (lattice + stream) and pdfminer.six to detect section titles, filters valid station rows, maps columns to a fixed schema, and writes per‑year CSVs into data/extracted_csv_by_section/.

preprocess_merge.py

Role: Data cleaning and standardization.

Takes the raw per‑year CSVs from extracted_csv_by_section/, normalizes text, converts numeric min/max columns to floats, drops rows with no numeric data, computes representative means (pH, DO, BOD, conductivity, nitrate), and derives clean waterbody names.

Outputs standardized per‑year CSVs plus a single merged master file data/processed/water_quality_2016_2023.csv.

river_analysis_and_plots.py

Role: River‑wise analysis and visualization.

Loads the master processed CSV and performs safety assessment for a chosen river using two criteria: BIS pH gate and composite raw‑water gate (pH, DO, BOD, fecal and total coliform).

Produces per‑river CSVs (all records and latest status), a JSON summary report, and multiple PNG plots (trends, heatmaps, compliance over time, state‑wise status, parameter correlation) stored under outputs/river_reports/.

requirements.txt

Role: Lists Python dependencies needed to run all the above scripts (scraping, PDF extraction, cleaning, analysis, plotting).

data/ 

raw_pdfs/

Optional folder to keep any manually downloaded sample PDFs (not required by the automated pipeline).

CPCB_Rivers_Main_Data/

Destination for PDFs downloaded by webscraping.py.

One file per year, e.g., 2016.pdf, 2017.pdf, …, 2023.pdf.

extracted_csv_by_section/

Contains intermediate CSVs created by extract_by_section.py, one per year.

Each row corresponds to a station/section with min/max values for different parameters.

processed/

Contains cleaned, standardized CSVs created by preprocess_merge.py.

Includes per‑year standardized files and the final merged dataset water_quality_2016_2023.csv used by the analysis script and by the backend team.
...
For each analyzed river, stores:
...
Several PNG plots and supporting CSVs (trends, heatmaps, compliance, correlation).